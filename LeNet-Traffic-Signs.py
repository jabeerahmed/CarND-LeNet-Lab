...
signs = read_signs('signnames.csv')
num_classes=len(signs)
num_data = {'train': len(train['labels']), 'test': len(test['labels']), 'valid': len(valid['labels'])}
...
print('Image Dimensions: ' + str(image_shape))
print()
...
def LeNet(x):    
    ...
    return fully_connected(fc2_relu, 43, mu, sigma)   
...
#logits = MT.LeNet(x, num_classes)
#cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=one_hot_y)
#loss_operation = tf.reduce_mean(cross_entropy)
#optimizer = tf.train.AdamOptimizer(learning_rate = rate)
#training_operation = optimizer.minimize(loss_operation)
logits, training_operation = MT.InitTrainPipeline(x, num_classes, one_hot_y, network=MT.LeNet, rate=rate)
...
_, accuracy_operation, saver = MT.InitEvalPipeline(logits, one_hot_y)
...
#        validation_accuracy = evaluate(X_valid, y_valid)
        validation_accuracy = MT.Evaluate(X_valid, y_valid, x, y, accuracy_operation, BATCH_SIZE)
